...
namespace TrueMyth
{
    ...
    public static class Result
    {
        ...
        public static IResult<TSelected, TError> Select<TValue, TSelected, TError>(Func<TValue, TSelected> selector, IResult<TValue, TError> result) 
            => IsOk(result)
                ? new Ok<TSelected, TError>(selector(UnsafelyUnwrap(result)))
                : result as IResult<TSelected, TError>;

        public static IResult<TSelected, TError> SelectMany<TValue, TSelected, TError>(Func<TValue, IResult<TSelected, TError>> selector, IResult<TValue, TError> result) 
            => IsOk(result)
                ? selector(UnsafelyUnwrap(result))
                : result as IResult<TSelected, TError>;

        public static IResult<TValue, TSelectedError> SelectErr<TValue, TError, TSelectedError>(Func<TError, TSelectedError> selector, IResult<TValue, TError> result) 
            => IsErr(result)
    ...
    public class Ok<TValue, TError> : IResult<TValue, TError>, IEquatable<IResult<TValue, TError>>
    {
